                    ...
                    <div class="card-body">
                      <form  method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>?model_id=<?php echo $get_model_id; ?>">
                        <div class="row">
                          ...
                          <label>Type</label>
                          <input type='text' class='form-control' value="<?php echo $type; ?>" disabled> <br>

                           <button type="buttom" name="save" id="save" class="btn btn-primary" onclick="this.disabled=true;this.value='Submitting...'; this.form.submit();" >Save</button>
                    </form>
                        </div>
                          ...
                          if($result = mysqli_query($link, $query)){
                            if(mysqli_num_rows($result) > 0){
                              ...
                              while($row = mysqli_fetch_array($result)){
                                $packlist_model = $row['pack_list_model']; 
                                $packlistQty = $row['pack_list_qty'];
                                $packId = $row['pack_list_id'];
                                $ctr++; ?>


                                <tr>
                                  <td><?php echo $ctr;?></td>
                                  <td><?php echo $packlist_model;?></td>
                                  <td><?php echo $packlistQty;?></td>
                                  <td>
                                    <a href="#" title="Update Record" data-toggle="modal" data-target="#modal-sm<?php echo $row['pack_list_id'];?>" >Update</a>

                                    <!--=====================================update modal==============================================-->
                                    <div class="modal fade" id="modal-sm<?php echo $packId; ?>" style="display: none;" aria-hidden="true">
                                          ...
                                          <div class="modal-body">
                                            <form  method="POST" action="package-list-update.php" id="mForm">
                                              <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" placeholder="Package Name" name="package_name" oninput="upperCase(this)" maxlength="20" value="<?php echo $packlist_model; ?>" id='model'><br>
                                              </div>

                                              <div class="form-group">
                                                <label>Quantity<span style="color: Red;">*</span></label>
                                                <input type="number" class="form-control" placeholder="Quantity" name="quantity" value="<?php echo $packlistQty; ?>" id='qty'>
                                              </div>

                                              <input type="number" name="model_id" id='id' value="<?php echo $packId; ?>">      
